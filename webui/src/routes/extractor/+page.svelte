<script lang="ts">
  // Extractor 开发工具页面：调用 /extractor/{url}，验证正文提取效果
  let urlInput = '';
  let headful = false;
...
<svelte:head><title>Extract - RssAny</title></svelte:head>
...
<main class="main">
  ...
  <div class="form-wrap">
    <form on:submit|preventDefault={go}>
      <div class="url-row">
        <input type="url" bind:value={urlInput} placeholder="输入文章详情页地址…" required autocomplete="url" />
