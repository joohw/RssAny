...
- **代理**：通过全局 `proxy.json` 配置文件或环境变量 `HTTP_PROXY`/`HTTPS_PROXY` 设置，在 `RequestConfig.proxy` 中传入；Puppeteer 启动时添加 `--proxy-server`。示例：无认证 `http://127.0.0.1:7890`、`socks5://127.0.0.1:1080`；需账号密码时 `http://user:pass@host:port`（代理返回 407 时自动用 `page.authenticate` 应答）。
