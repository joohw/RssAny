...
│   │   └── scheduler/       定时抓取
│   ├── writer/             写文件模块（条目以 Markdown 落盘到配置目录）
│   ├── types/               FeedItem、译文等
...
├── config.json             enrich、writer（写文件）等
...
### pipeline（入库前处理链）
- **职责**：对条目执行可配置的处理函数链，支持翻译、打标签等。
- **接口**：每个插件实现 `match` 与 `run(item, ctx)`，必须按 FeedItem 接口输出。
...
- **配置**：在 `.rssany/config.json` 中增加 `writer` 块：`enabled`（是否启用，默认 `false`）、`repoPath`（写文件目标目录，支持相对或绝对路径）。环境变量 `WRITER_REPO_PATH` 可覆盖 `repoPath`；未配置或 `enabled: false` 时不写文件。
...
- **Markdown 格式**：UTF-8；YAML frontmatter（`---` 起止）+ 正文。frontmatter 推荐字段：`id`、`url`、`source_url`、`sources`、`title`、`author`、`pub_date`、`fetched_at`。正文第一行建议一级标题 `# <title>`，随后摘要与正文（由 `content` 等生成）。
...
| **pipeline** | `plugins/pipeline/`、`.rssany/plugins/pipeline/` | 二次加工（翻译、打标签等） | 每条仅一次：有 enrich 在补全后；无 enrich 在入库后 |
...
- **pipeline**：决定「入库前如何加工」；通过 `match` 与 `run(item, ctx)` 对条目做统一处理。

**分布式爬虫**：若需将抓取任务分散到多机，可在 pipeline 插件中实现：对条目执行 `fetch(url, { method: 'POST', body: JSON.stringify(item) })` 投递到其他节点的接收路由，由远端完成入库或二次处理；本地可选择跳过入库或仅做轻量记录。
