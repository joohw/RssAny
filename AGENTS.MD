...
│   ├── config.json         可选：enrich、signal 等配置
...
- **Signal 投递**：当 `writeDb` 且 `.rssany/config.json` 中 `signal.enabled` 为 true 时，同步将条目以 Markdown 写入 Signal 仓库目录（仅写文件，不 git commit），见 CONVENTIONS.md「Signal 仓库投递」
