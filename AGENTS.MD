...
├── cache/                  运行时缓存（fetched、parsed、extracted、feeds、domains、browser_data）
├── plugins/                用户插件
...
### feeder（RSS 生成核心）
- 根据 URL 生成 RSS：缓存 → getSource → fetchItems → **pipeline** → upsertItems → buildRssXml；有 enrichItem 则提交队列，完成后 **pipeline** → updateItemContent 并更新缓存。支持 `lng` 译文输出。writeDb 时写库并可选用写文件模块将条目落盘（见下文「写文件模块」）。

### pipeline（入库前处理链）
- **职责**：在入库前对条目执行可配置的处理函数链，支持翻译、打标签等。
- **接口**：每个步骤实现 `(item: FeedItem, ctx: PipelineContext) => Promise<FeedItem>`，必须按 FeedItem 接口输出。
- **配置**：`.rssany/config.json` 中 `pipeline.enabled`、`pipeline.steps`（如 `["tag", "translate"]`）；默认 `enabled: false`。
- **执行时机**：列表入库前（`isEnriched: false`）、详情补全后（`isEnriched: true`）。
...
1. **RSS 生成 + 入库流程**：
   ...
   URL → getSource() → preCheckAuth() → fetchItems()
     → pipeline（若启用）→ upsertItems() → buildRssXml()
     → [后台] enrichItem() → pipeline（若启用）→ updateItemContent() → 更新缓存
   ```
